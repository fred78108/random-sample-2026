...
- [x] **G — Score prediction & weekly extremes** *(depends on F — only built for whichever config F promotes)* — predict a final score for each team in every game, and surface the single team predicted to score highest and the single team predicted to score lowest across the whole week's slate (display/analysis only — never feeds confidence ranking or point scoring)
- [x] **H — Prompt & model sweep** — mid-season improvement pass allowed by Rules 2/7: held `agent_design` fixed at the promoted `single_analyst` (Rule 4 forbids a new agent type once the season has started) and swept 2 new prompt variants (structured-factor, market-anchored) × 2 already-pulled models against the promoted config, screened on 2025. **No candidate beat the promoted config** — the market-anchored prompt underperformed on both models, and the larger `qwen3.5:397b-cloud` model wasn't a reliable upgrade over `glm-5.3-flash:cloud`. User decision: conclude without promoting; `config.toml` unchanged. See [specs/pickem-agent/TASKS.md](specs/pickem-agent/TASKS.md#milestone-h--prompt--model-sweep-in-season-improvement).

`config.toml` still holds Milestone F's promoted config (`single_analyst` / `single_analyst_v1` / `glm-5.3-flash:cloud` / `rich`) — Milestone H's sweep didn't find a config that clears the promotion bar.
